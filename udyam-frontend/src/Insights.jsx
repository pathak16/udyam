...
function Insights({ data, onSummaryGenerated }) {
  ...
  // ✅ FIX: Use useEffect to send summary to parent safely
  useEffect(() => {
    if (onSummaryGenerated) {
      onSummaryGenerated(summary);
    }
  }, [JSON.stringify(summary)]); // stringified for deep change detection
    ...
    <div style={{ marginTop: "2rem" }}>
      ...
      <ul>
        {summary.map((item) => (
          ...
          <strong style={{ fontSize: "1.1rem" }}>📍 {item.area}</strong><br />
          🏪 <strong>Businesses:</strong> {item.count}<br />
          ⭐ <strong>Avg Rating:</strong> {item.avgRating}<br />
          📊 <strong>Status:</strong> {item.label}<br />
